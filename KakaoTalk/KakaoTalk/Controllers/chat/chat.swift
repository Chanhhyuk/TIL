...
class chat: UIViewController, UITableViewDataSource {    // UITableViewDataSource 프로토콜 사용
    ...
    private func naviView() {
        ...
        let chatAdd = UIButton()
        chatAdd.setImage(UIImage(named: "chatAdd"), for: .normal)
        ...
        let chatAddBtn = UIBarButtonItem(customView: chatAdd)
        let talkMusicBtn = UIBarButtonItem(customView: talkMusic)
        let settingBtn = UIBarButtonItem(customView: setting)
        navigationItem.rightBarButtonItems = [settingBtn, talkMusicBtn, chatAddBtn, searchBtn]
